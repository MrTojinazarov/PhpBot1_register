        ...
        try {
            ...
            if (Cache::get("register_step_{$chat_id}") === 'confirmation_code') {
                if ($text === Cache::get("confirmation_code_{$chat_id}")) {
                    ...
                    Cache::put("register_step_{$chat_id}", 'img');
            ...
            if (Cache::get("register_step_{$chat_id}") === 'img') {
                if ($photo) {
                    ...
                    if (isset($response['result']['file_path'])) {
                        ...
                        $img_name = uniqid() . '.jpg';
                        $img_content = file_get_contents($download_url);

                        if ($img_content) {
                            Storage::disk('public')->put("uploads/{$img_name}", $img_content);
